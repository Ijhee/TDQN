...
class TradingSimulator:
    ...
    def plotEntireTrading(self, trainingEnv, testingEnv, stockName):
        """
        ...
        ax1.plot(data.loc[data['Action'] == 2.0].index, 
                 data['Close'][data['Action'] == 2.0],
        ...
        ax2.plot(data.loc[data['Action'] == -1.0].index, 
                 data['Money'][data['Action'] == -1.0],
                 'v', markersize=5, color='blue', label='Short')
        ax2.plot(data.loc[data['Action'] == 2.0].index, 
                 data['Money'][data['Action'] == 2.0],
                 '^', markersize=5, color='green', label='Long 100')

        # Plot the vertical line separating the training and testing datasets
        ...
        ax1.legend(["Price", "Long 50", "Short", "Long 100", "Train/Test separation"])
        ax2.legend(["Capital", "Long 50", "Short", "Long 100", "Train/Test separation"])
        plt.savefig(''.join(['Figures/', str(trainingEnv.marketSymbol), '_TrainingTestingRendering', '.png'])) 
        data.to_csv(stockName+'_Result.csv',index=False)
        #plt.show()
    ...
    def simulateNewStrategy(self, strategyName, stockName,
        ...
        if rendering:
            self.plotEntireTrading(trainingEnv, testingEnv, stockName)
